...
const SEARCH_ITEMS: SettingsSearchItem[] = [
  ...
  { id: 'advanced-customer-search', label: 'Advanced customer search', description: 'Find customers using combined filters', keywords: 'brand model location nearby service amc never gst date filter', group: 'Customers & work', icon: Users, destination: { type: 'panel', panel: 'advanced-search' } },
  ...
  { id: 'technician-locations', label: 'Technician locations', description: 'Last known pin, plus worked hours and travel km after 9 PM', keywords: 'staff gps map current location hours km travel distance worked', group: 'Technicians', icon: MapPin, destination: { type: 'section', section: 'technician-locations' } },
  ...
  { id: 'service-hubs', label: 'Location hubs', description: 'Areas where website and WhatsApp booking is allowed', keywords: 'hub hubs service area coverage radius map pincode book location booking zone hsr btm bellandur polygon', group: 'Customers & work', icon: MapPin, destination: { type: 'panel', panel: 'service-hubs' }, adminOnly: true },
  { id: 'office-location', label: 'Office location', description: 'Shop pin for technician travel km (avoid tolls)', keywords: 'dashboard office km travel distance maps start work', group: 'App & data', icon: MapPin, destination: { type: 'section', section: 'dashboard' }, adminOnly: true },
  { id: 'live-booking-banner', label: 'Live booking banner', description: 'Show or hide the live website booking banner on the dashboard', keywords: 'dashboard website book intent live banner switch hide', group: 'App & data', icon: Megaphone, destination: { type: 'section', section: 'dashboard' }, adminOnly: true },
  { id: 'add-customer-universal-resume', label: 'Add Customer resume on other phones', description: 'Follow unfinished Add Customer drafts across phones, or keep them on this device only', keywords: 'dashboard add customer draft resume universal localstorage phone sync switch', group: 'App & data', icon: UserPlus, destination: { type: 'section', section: 'dashboard' }, adminOnly: true },
  { id: 'pdf-compression', label: 'Compress PDFs', description: 'Turn iLovePDF compression on or off for all document PDFs', keywords: 'dashboard pdf quote quotation bill invoice amc warranty size ilovepdf credits switch', group: 'App & data', icon: FileText, destination: { type: 'section', section: 'dashboard' }, adminOnly: true },
  ...
  { id: 'storage', label: 'Storage usage', description: 'Postgres, R2, Cloudinary, iLovePDF, and Google Maps billing', keywords: 'database space files media ilovepdf credits compress pdf google maps billing api usage places geocoding', group: 'App & data', icon: Database, destination: { type: 'panel', panel: 'db-storage' }, adminOnly: true },
  ...
  { id: 'passkeys', label: 'Passkeys', description: 'Face ID or fingerprint sign-in', keywords: 'security webauthn biometric fingerprint face id login passwordless', group: 'App & data', icon: Fingerprint, destination: { type: 'section', section: 'passkeys' }, adminOnly: true },
];
...
export function SettingsCommandPalette() {
  ...
  const renderItem = (item: SettingsSearchItem) => (
    <CommandItem
      key={item.id}
      value={`${item.label} ${item.description} ${item.keywords} ${item.id}`}
