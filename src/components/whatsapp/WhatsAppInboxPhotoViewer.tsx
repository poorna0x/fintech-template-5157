...
const PSWP_CSS = `
.pswp.pswp-inbox { --pswp-bg: #1a1f24; z-index: 9999 !important; }
.pswp-inbox .pswp__bg { background: #1a1f24 !important; }
...
export function WhatsAppInboxPhotoViewer({
  ...
  return createPortal(
    <>
      ...
      >
        <header
          className="pointer-events-auto flex shrink-0 items-center gap-2 border-b border-white/10 bg-[#22282e] px-3 pb-2 pt-[max(0.5rem,env(safe-area-inset-top))]"
        >
          <Button
            type="button"
            size="icon"
        ...
        {!ready ? (
          <div className="pointer-events-none flex flex-1 items-center justify-center bg-[#1a1f24]">
            <Loader2 className="h-8 w-8 animate-spin text-[#7a746c]" />
          </div>
        ) : null}
