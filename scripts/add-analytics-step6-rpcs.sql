-- Step 6: slim technicians list for CRM Analytics + slim metadata in delete preview.
-- Requires: public.is_admin_user(), preview_website_analytics_delete from add-website-analytics-delete-rpc.sql
...
CREATE OR REPLACE FUNCTION public.get_technicians_for_analytics()
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.get_technicians_for_analytics() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_technicians_for_analytics() TO authenticated;

-- Patch delete preview: reuse slim metadata helper (same fields as recent-events list).
