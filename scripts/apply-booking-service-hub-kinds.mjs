/**
 * Apply booking hub service_kind column.
 *   node scripts/apply-booking-service-hub-kinds.mjs
 */
import fs from 'fs';
...
const sqlPath = path.join(root, 'scripts/add-booking-service-hub-kinds.sql');
...
try {
  ...
  `);
  console.log('OK: applied scripts/add-booking-service-hub-kinds.sql');
  console.log('service_kind column:', rows[0]?.has_kind);
} finally {
  await client.end();
}
