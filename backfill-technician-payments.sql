-- Backfill Technician Payments for Existing Completed Jobs
-- This function creates payment records for jobs that were completed before the trigger was set up

-- Function to backfill payment records for completed jobs
CREATE OR REPLACE FUNCTION backfill_technician_payments()
RETURNS TABLE(
    created_count INTEGER,
    skipped_count INTEGER,
    error_count INTEGER
) 
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
    job_record RECORD;
    created_count INTEGER := 0;
    skipped_count INTEGER := 0;
    error_count INTEGER := 0;
BEGIN
    -- Loop through all completed jobs that have assigned technicians and payment amounts
    FOR job_record IN 
        SELECT 
            j.id as job_id,
            j.assigned_technician_id,
            j.payment_amount,
            j.actual_cost,
            j.status
        FROM jobs j
        WHERE j.status = 'COMPLETED'
          AND j.assigned_technician_id IS NOT NULL
          AND (j.payment_amount > 0 OR j.actual_cost > 0)
    LOOP
        BEGIN
            -- Check if payment record already exists
            IF NOT EXISTS (
                SELECT 1 FROM technician_payments 
                WHERE job_id = job_record.job_id
            ) THEN
                -- Create payment record
                INSERT INTO technician_payments (
                    technician_id,
                    job_id,
                    bill_amount,
                    commission_percentage,
                    commission_amount,
                    payment_status
                ) VALUES (
                    job_record.assigned_technician_id,
                    job_record.job_id,
                    COALESCE(job_record.payment_amount, job_record.actual_cost, 0),
                    10.00,
                    COALESCE(job_record.payment_amount, job_record.actual_cost, 0) * 0.10,
                    'PENDING'
                );
                created_count := created_count + 1;
            ELSE
                skipped_count := skipped_count + 1;
            END IF;
        EXCEPTION WHEN OTHERS THEN
            error_count := error_count + 1;
            -- Log error but continue processing
            RAISE WARNING 'Error creating payment for job %: %', job_record.job_id, SQLERRM;
        END;
    END LOOP;
    
    RETURN QUERY SELECT created_count, skipped_count, error_count;
END;
$$ LANGUAGE plpgsql;

-- Run the backfill function
SELECT * FROM backfill_technician_payments();

