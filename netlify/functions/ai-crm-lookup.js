...
jobs(id uuid, job_number text, customer_id uuid, status text,
  ...
  completed_at timestamptz, start_time timestamptz, end_time timestamptz, scheduled_date date,
...
- Job duration = COALESCE(end_time, completed_at) - start_time (COMPLETED jobs only)
- Do NOT filter to today unless the user said today / this week / this month
- Day of week: TO_CHAR(scheduled_date, 'FMDay') plus EXTRACT(DOW FROM scheduled_date)
- Hour of day (IST): EXTRACT(HOUR FROM (start_time AT TIME ZONE 'Asia/Kolkata'))
`.trim();
...
function formatSqlCell(key, value) {
  if (value == null || value === '') return null;
  const k = String(key || '').toLowerCase();
  const asText = String(value);
  if ((/^id$|_id$/.test(k) || k === 'id') && /^[0-9a-f]{8}-[0-9a-f-]{27,}$/i.test(asText)) {
    return null;
  }
  if (/day_of_week|dow/.test(k) && Number.isFinite(Number(value))) {
    const n = Number(value);
    if (n >= 0 && n <= 6) return IST_DAY_NAMES[n];
  }
  if (/^day_name|weekday/.test(k)) {
    return String(value).trim();
  }
  if (/hour/.test(k) && Number.isFinite(Number(value)) && Number(value) >= 0 && Number(value) <= 23) {
    return formatHourLabel(value);
  }
  if (/duration_min|minutes/.test(k) && Number.isFinite(Number(value))) {
  ...
  if (/amount|cost|total|revenue|expense|payment/.test(k) && Number.isFinite(Number(value))) {
...
function formatSqlRows(rows, label) {
  ...
  const skipKey = (k) => /day_of_week|dow/.test(k) && keys.some((other) => /day_name|weekday/.test(other));
  const lines = rows.map((row, index) => {
    const parts = keys
      .filter((k) => !skipKey(k))
      .map((k) => formatSqlCell(k, row[k]))
      .filter(Boolean);
...
function detectJobDurationRanking(message) {
  const lower = String(message || '').toLowerCase();
  if (!/\b(?:shortest|longest|fastest|slowest)\b/.test(lower)) return null;
  if (!/\b(?:job|visit|completion|completed|duration|time)\b/.test(lower)) return null;
  return /\b(?:longest|slowest)\b/.test(lower) ? 'DESC' : 'ASC';
}

function jobDurationSql(direction) {
  const order = direction === 'DESC' ? 'DESC' : 'ASC';
  return `
SELECT job_number,
       ROUND(EXTRACT(EPOCH FROM (COALESCE(end_time, completed_at) - start_time)) / 60)::int AS duration_minutes,
       scheduled_date
FROM jobs
...
function extractRadiusKm(message) {
  const text = String(message || '');
  // Last explicit radius wins ("… maps URL … 50m surrounding").
  // Do not treat the "m" in "maps" / "me" / "month" as metres.
  const metreRe = /(?<![.\d])(\d+(?:\.\d+)?)\s*(?:meters?|metres?|mtr|mtrs|m(?![a-z]))/gi;
  let metreKm = null;
  let match;
  while ((match = metreRe.exec(text))) {
    const km = parseFloat(match[1]) / 1000;
    if (Number.isFinite(km) && km > 0) metreKm = Math.min(km, 50);
  }
  if (metreKm != null) return metreKm;
  const kmRe = /(\d+(?:\.\d+)?)\s*(?:km|kilometers?|kilometres?)\b/gi;
  let kmVal = null;
  while ((match = kmRe.exec(text))) {
    const km = parseFloat(match[1]);
    if (Number.isFinite(km) && km > 0) kmVal = Math.min(km, 50);
  }
  if (kmVal != null) return kmVal;
  return 5;
}
