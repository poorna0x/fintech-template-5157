...
// Admin-only HTTP wrapper around location-ping-helper.
...
exports.handler = async (event) => {
  ...
  const result = await sendTechnicianLocationPing(db, technicianId, { force: true });
  if (result.reason === 'lookup_failed' || result.reason === 'push_failed') {
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: result.reason === 'push_failed' ? 'Push send failed' : 'Lookup failed' }),
    };
  }
  return {
    statusCode: 200,
