-- ROLLBACK: Undo 20250303000000_rls_initplan_and_duplicate_index.sql
-- Run this only if the optimization migration caused issues:
--   psql "$DATABASE_URL" -f supabase/scripts/rollback_rls_initplan_and_duplicate_index.sql
--
-- This restores: (1) the duplicate index, (2) RLS policies to auth.role()/auth.uid() without (select ...).

-- ---------------------------------------------------------------------------
-- 1. Re-create the dropped index
-- ---------------------------------------------------------------------------
CREATE INDEX IF NOT EXISTS idx_follow_ups_follow_up_date ON public.follow_ups USING btree (follow_up_date);

-- ---------------------------------------------------------------------------
-- 2. RLS policies: revert to auth.role() / auth.uid() (no select wrapper)
-- ---------------------------------------------------------------------------
