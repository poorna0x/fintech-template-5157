...
async function testSecurity() {
  ...
  console.log('-'.repeat(70));
  try {
    // Test missing required fields (empty body) - use hash endpoint to avoid rate limit
    const missingFields = await makeRequest('POST', ENDPOINTS.hashPassword, null, {
      'Content-Type': 'application/json'
    });
    ...
    } else if (missingFields.status === 429) {
      results.warnings.push('Input Validation: Rate limited (test skipped)');
      console.log('   ⚠️  Rate limited - cannot test input validation');
    } else {
      results.failed.push('Input Validation: Missing fields accepted');
      console.log(`   ❌ Missing fields NOT rejected (status: ${missingFields.status})`);
    }
  } catch (error) {
  ...
  try {
    ...
    for (const [header, expected] of Object.entries(securityHeaders)) {
      ...
      if (headerValue && headerValue.toString().toLowerCase().includes(expected.toLowerCase())) {
        headersFound++;
    ...
    if (headersFound >= 3) {
      ...
      console.log('   ⚠️  No security headers detected (may be case-sensitive or missing)');
    }
  } catch (error) {
